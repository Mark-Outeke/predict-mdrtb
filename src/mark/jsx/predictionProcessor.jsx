...
import { useTrackedEntity } from 'mark/jsx/TrackedEntityContext'; // Import your existing context
import tracker from 'mark/api/tracker';
import Header from 'mark/jsx/Header';
...
const PredictionComponent = () => {
  ...
  const [reverseDataElementDisplayNames, setReverseDataElementDisplayNames] = useState({}); // New state for reverse mapping
  ...
  useEffect(() => {
    const fetchDataElementDisplayNames = async () => {
      ...
      try {
        ...
        const reverseDisplayNameMapping = {}; // New mapping for reverse lookup
 ...
 // console.log('trackedEntityData:', trackedEntityData);
      ...
      if (model) {
        ...
        // Loop through each tensor input and make predictions
          ...
          for (let i = 0; i < tensorInputs.length; i++) {
           ...
           const runIntegratedGradients = async (model, reshapedIGInput, steps = 10) => {
...
// Map the sortedAveragedIGValues to tracked entity data values
const trackedValues = sortedAveragedIGValues.map(feature => {
  const dataElementId = reverseDataElementDisplayNames[feature.featureId] || null;

  let trackedValue = 'N/A'; // Default value

  // Check each enrollment
  if (dataElementId) {
    for (const enrollment of trackedEntityData.enrollments || []) {
      // Iterate through events in this enrollment
      for (const event of enrollment.events || []) {
        // Find the data value matching the dataElementId
        const dataValue = event.dataValues.find(dataValue => 
          dataValue.dataElement === dataElementId
        );

        if (dataValue) {
          trackedValue = dataValue.value; // Update trackedValue if found
          break; // Break inner loop if value found
        }
      }
      // Break outer loop if value has already been found
      if (trackedValue !== 'N/A') break; 
    }
  }
  ...
  return {
    ...
    trackedValue // Return the found tracked value
  };
});
...
return (
  ...
  <div className="card-body">
    ...
    <h5 className="text-center">Sorted Contributing Factors and their Values</h5>
    <h6> These expalin why the patient is likely to develop MDRTB.They shoud be used to guide clinical decision making and to inform treatment decisions.</h6>
      <table className="table table-bordered mt-3">
        <thead>
          <tr>
            <th>Feature</th>
            <th>Contributing Factor Value</th>
          </tr>
        </thead>
        <tbody>
          {trackedValues.map(({ featureId, trackedValue }) => (
            <tr key={featureId}>
              <td>{featureId}</td>
              <td>{trackedValue}</td>
            </tr>
